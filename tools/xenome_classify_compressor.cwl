...
# Compress all FASTQs produced by xenome, in place, in the same working dir.
# gzip is the compressor available in the xenome2 image.
- position: 6
  shellQuote: false
  valueFrom: >-
    && gzip -f *.fastq
...
outputs:
  graft_fastqs:
    type: 'File[]'
    outputBinding:
      glob: '*$(inputs.graft_name)*.fastq.gz'
    sbg:inheritMetadataFrom: fastq_reads
  host_fastqs:
    type: 'File[]'
    outputBinding:
      glob: '*$(inputs.host_name)*.fastq.gz'
    sbg:inheritMetadataFrom: fastq_reads
  output_stats:
    type: File
    outputBinding:
